...
- [x] Merging an example PR that adds a new source rule does NOT modify `/dist/`. The dist tree is regenerated only on manual release. — **Mechanical guards in place** (no auto-regenerate-on-PR trigger anywhere; `release.yml` is `workflow_dispatch:` only) AND **live-PR-experiment satisfied** by PR #1 (`fix/release/reconcile-bot-slug`, merged 2026-05-20 as `ff40908`). That PR touched 13 non-`dist/` files; the `dist-protection-lint` job correctly short-circuited to "PR does not modify any dist/ paths. Pass." (run `26152413195`) and the merge left `dist/stacks/` byte-for-byte unchanged from `v0.1.0`. A future content-only PR that adds a source rule is still useful end-to-end evidence (the `dist-protection-lint` lint already proves the mechanical guard), and the next genuine source-rule PR can confirm — Phase 9 will produce many of these as pilot feedback lands.
- [x] Manually dispatching `release.yml` produces a single commit authored by `@engineering-standards-bot`, a `v<X.Y.Z>` tag, a populated `dist/CHANGELOG.md`, and a GitHub Release. — Satisfied by the `v0.1.0` dispatch on 2026-05-20 06:30 UTC: workflow run `26145610720`, commit `28c02fb5a4f21ac3fc371833a9d69e346be6ae5f`, GitHub author login `engineering-standards-bot[bot]` (numeric user ID `286238545`), tag `v0.1.0`, `dist/CHANGELOG.md` populated with the first-release rule-by-rule index (1980-byte release body), GitHub Release "Engineering Standards v0.1.0" published. The only post-event reconciliation needed was the bot slug (PR #1) — the dispatch itself succeeded on first try.
- [x] A test PR by a human that touches a file under `dist/` is blocked by the `dist-protection-lint` status check with a clear error message. — **Mechanical guard locked** by `tests/test_phase7_end_to_end.py::test_dist_protection_lint_failure_message_cites_adr_0004` AND **end-to-end-runnable** verified by PR #1 (the lint actually ran on a PR head and reached its "Pass" branch — run `26152413195`). The PR-#1 run also surfaced the `--depth=0` hotfix (`0d61333`), which is now guarded by `tests/test_phase7_end_to_end.py::test_dist_protection_lint_does_not_pass_invalid_depth_zero_to_git_fetch`. A deliberate human-edits-`dist/` PR remains useful as the next live experiment, but the mechanical contract (right regex, right author allowlist, right exit code, right failure message) is fully pinned.
- [x] Branch protection on `main` is verified via screenshot in the closing PR for Phase 7. — Applied as a GitHub **Ruleset** (`gh api POST /repos/.../rulesets`) rather than classic branch protection, because `dgwadkar/common-engineering-standards` is a User-owned repo and classic-protection `restrictions.apps` is org-only. Ruleset `protect-main` (id `16632798`, enforcement `active`) targets `~DEFAULT_BRANCH`, enforces all five required status checks listed in `docs/branch-protection-config.md` §2.2, and has bypass actors `RepositoryRole admin (5)` + `Integration engineering-standards-bot (3777459)` so the release workflow can push directly to `main`. API readout is more durable evidence than a UI screenshot; see `docs/branch-protection-config.md` §5 for the live state and the verification query. The classic-protection AC text remains in §2 of that doc for completeness; the §5 reconciliation note explains the Ruleset substitution.
- [x] `git log --author=engineering-standards-bot dist/` returns the complete history of dist regenerations and nothing else. — Satisfied as of the `v0.1.0` dispatch: `git log --pretty='%H %an' main -- dist/` returns exactly one entry, `28c02fb5a4 standards-bot[bot]` (note: pre-slug-reconciliation literal in `commit.author.name`; the GitHub-side author login is `engineering-standards-bot[bot]` per the post-Phase-7 reconciliation in `branch-protection-config.md` §5). From `v0.1.1` onwards both fields align. The audit query in `docs/release-bot-setup.md` §6 remains the standing quarterly procedure.
