...
2. **PRs touching `/dist/` are produced ONLY by the release workflow.** Humans (and AI agents
   ...
   by `release.yml` running under the `@engineering-standards-bot` GitHub App identity.
...
| Release identity | `@engineering-standards-bot` GitHub App | Phase 7. |
...
| `dist/**` | **NO.** Regenerated by `release.yml` under `@engineering-standards-bot` only. | n/a |
